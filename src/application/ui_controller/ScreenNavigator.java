...
public class ScreenNavigator
{
	...
	/**
	 * Loads the screen specified by the fxml file into the screenHolder pane of the main application layout.
	 *
	 * Previously loaded screen for the same fxml file are not cached. The fxml is loaded anew and a new screen node hierarchy generated every time this method is invoked.
	 *
	 * A more sophisticated load function could potentially add some enhancements or optimizations, for example: cache FXMLLoaders cache loaded screen nodes, so they can be recalled or reused allow a user to specify screen node reuse or new creation allow back and forward history like a browser
	 *
	 * @param fxml
